...
from src.models.generator import FCGenerator
...
class S2BModel(pl.LightningModule):
    def __init__(self,
        ...
        self.encoder.load_state_dict(torch.load('pretrained/jangnan.pt'))

        self.net_G = FCGenerator(fc1_dim, fc2_dim)
        self.criterion_MSE = nn.MSELoss(reduction='sum')

        self.net_D = Discriminator(in_channels=65)
        self.criterion_GAN = GANLoss()
    
    def forward(self, x, x_length, y, y_length):
        enc_out = self.encoder(x, x_length)
        enc_out = enc_out.permute(1, 0, 2).contiguous()
        speech_features = self.interpolate_features(enc_out, 50, 60, output_len= len(y[0]))
        pred_blendshape = self.net_G(speech_features)

        return pred_blendshape, speech_features
    ...
    def masking_preds(self, out, y, y_length):
        ...
        return masked_out, chopped_y # B * 1 * T * F
    ...
    def training_step(self, batch, batch_idx, optimizer_idx):
        ...
        out, speech_features = self(x, x_length, y, y_length)

        masked_out, chopped_y = self.masking_preds(out, y, y_length)
        masked_features = self.masking_features(speech_features, y_length, masked_out)
