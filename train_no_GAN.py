...
def main(args):
    ...
    data_loader = FaceDataModule(os.path.join(args.root, 'audio_ggongggong.pt')
                                ,os.path.join(args.root, 'shape_ggongggong.pt'), 16, 64)
    print(f'Loaded data from {args.root}')
    
    print(args.pretrained)
    if args.pretrained:
        model = GGomYangModel.load_from_checkpoint(args.pretrained)
    else:
        model = GGomYangModel(
            lr=args.lr,
    ...
    checkpoint_callback = ModelCheckpoint(dirpath=f'{args.checkpoint_dir}/{args.name}-{dt_string}', save_top_k=2, monitor="val_loss", save_last=True)
